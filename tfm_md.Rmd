...
```{r}
circ <- as.data.frame(read.table("C:/Users/ainho/eccDNA/BED/SRR6315431_unknown_circle.bed",header = FALSE))
circ$quality <- cut(circ$score,breaks=c(-Inf,10,50,200,Inf),labels= c("Bad","Low", "Medium", "Good"),right = FALSE)

circ %>% group_by(quality) %>%count()
```
...
```{r}
  ...
  filter_circ <- reactive({
    filter_circ<- filter(df_circ, between(size_bp, input$size[1], input$size[2]), quality == input$quality)
  })
