...
result = getSequence(chromosome=2, start=19206729, end=19206729,type="entrezgene",seqType="gene_exon_intron", mart=ensembl)
...
my.dnastring <- as.data.frame(Biostrings::getSeq(BSgenome.Hsapiens.UCSC.hg19, "chr2", 19206729, 19206800))
...
selected_circle <- circ %>% dplyr::filter (chrom==2 & size_bp==126835743 & split_reads==60)

coords <- selected_circle %>% dplyr::select(chrom,start,end) %>% makeGRangesFromDataFrame 

coords1<-GRanges("chr18:13525372-13553682")

paste0("chr",selected_circle[,2],":",selected_circle[,3],"-",selected_circle[,4])
...
# Compute a good label
data$label <- paste0(data$base,":\n ",(round((data$fraction)*100,2)),"%")
ggplot(data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=base)) +
  geom_rect() +
  geom_label( x=3.5, aes(y=labelPosition, label=label), size=6) +
  scale_fill_manual(values=c("A"="#EC7063","T"="#16A085","G"="#F4D03F","C"="#1F618D")) +
  coord_polar(theta="y") +
  xlim(c(1, 4)) +
  theme_void() +
  theme(legend.position = "none")
